...
def label_check(y, hat_y=None, assign_task_type=None):
    ...
    if assign_task_type is not None:
        if assign_task_type in ["binaryclass", "multilabel", "multiclass", "regression"]:
            if assign_task_type == pre_task_type:
                ...
                raise Exception(
                    "current data not support the filled task-type {}, task-type {} is suggested".format(
                        assign_task_type, pre_task_type
                    )
                )
        else:
            raise Exception(
                "fill in correct task-type ['binaryclass', 'multilabel', 'multiclass', 'regression'], \
